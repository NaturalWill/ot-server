...
function capi_showmessage_by_data($msg, $code = 1, $data = array()) {
	obclean ();
	...
	$msgkey = $msgvalue = '';
	if (is_array ( $msg )) {
		$msgkey = empty ( $msg ['key'] ) ? '' : $msg ['key'];
		$msgvalue = empty ( $msg ['value'] ) ? '' : $msg ['value'];
	} elseif (is_string ( $msg )) {
		$msgkey = $msg;
	}
	include_once (S_ROOT . './language/lang_showmessage.php');
	if (isset ( $_SGLOBAL ['msglang'] [$msgkey] )) {
		$message = lang_replace ( $_SGLOBAL ['msglang'] [$msgkey], $msgvalue );
	} else {
		$message = $msgkey;
	}
	$r = array ();
...
function capi_runlog($file, $log, $halt = 0) {
	...
	$log = sgmdate ( 'Y-m-d H:i:s', $_SGLOBAL ['timestamp'] ) . "\t$type\t" . getonlineip () . "\t$_SGLOBAL[supe_uid]\t{$nowurl}\t" . str_replace ( array (
			"\r",
			"\n" 
	), array (
			' ',
...
function capi_ckrealname($type, $return = 0) {
	...
	if ($_SCONFIG ['realname'] && empty ( $_SGLOBAL ['member'] ['namestatus'] ) && empty ( $_SCONFIG ['name_allow' . $type] )) {
		if (empty ( $return ))
			capi_showmessage_by_data ( 'no_privilege_realname' );
		$result = false;
	}
	return $result;
}
...
function capi_cknewuser($return = 0) {
	...
	if ($_SCONFIG ['newusertime'] && $_SGLOBAL ['timestamp'] - $space ['dateline'] < $_SCONFIG ['newusertime'] * 3600) {
		if (empty ( $return ))
			capi_showmessage_by_data ( 'no_privilege_newusertime', 1, array (
					$_SCONFIG ['newusertime'] 
			) );
		$result = false;
	}
	// 需要上传头像
	if ($_SCONFIG ['need_avatar'] && empty ( $space ['avatar'] )) {
		if (empty ( $return ))
			capi_showmessage_by_data ( 'no_privilege_avatar' );
		$result = false;
	}
	// 强制新用户好友个数
	if ($_SCONFIG ['need_friendnum'] && $space ['friendnum'] < $_SCONFIG ['need_friendnum']) {
		if (empty ( $return ))
			capi_showmessage_by_data ( 'no_privilege_friendnum', 1, array (
					$_SCONFIG ['need_friendnum'] 
			) );
		$result = false;
	}
	// 强制新用户好友个数
	if ($_SCONFIG ['need_email'] && empty ( $space ['emailcheck'] )) {
		if (empty ( $return ))
			capi_showmessage_by_data ( 'no_privilege_email' );
		$result = false;
	}
	return $result;
}
...
function capi_realname($uid, $tmpspace = '') {
	...
	if (intval ( $tmpspace ['namestatus'] ))
		return $tmpspace ['name'];
	else
		return $tmpspace ['username'];
}
